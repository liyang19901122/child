...
/*
	[插件名称] 最新图片列表(图片+标题+介绍)
	[适用范围] 全站
*/

function PhotoPicMemo(){
	...
	global $fsql,$msql;
		...
		$picnum=1;
		$fsql->query("select * from {P}_photo_con where $scl order by $ord $sc limit 0,$shownums");
		...
		while($fsql->next_record()){
			...
			//参数列
			$i=1;
			$msql->query("select * from {P}_photo_prop where catid='$nowcatid' order by xuhao");
